 ...
 *
 * Decision data (candidates, observations, search metadata) is available as JSON via
 * `cc_last_decision_json_len` and `cc_copy_last_decision_json` after this returns
 * `CC_MOVE_PROVIDED`.
 */
...
/* Returns the length of the last decision JSON string in bytes, or 0 if no decision is
 * available. Use this to allocate a buffer before calling `cc_copy_last_decision_json`.
 */
uint32_t cc_last_decision_json_len(CCAsyncBot *bot);

/* Copies the last decision JSON string into the provided buffer.
 * Returns true if the copy succeeded, false if no decision is available or the buffer
 * is too small. The JSON is UTF-8 encoded and NOT null-terminated.
 */
bool cc_copy_last_decision_json(CCAsyncBot *bot, uint8_t *dst, uint32_t capacity);
