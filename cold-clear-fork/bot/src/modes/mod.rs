...
pub(crate) enum TaskResult<V, R, T> {
    NormalThink(normal::ThinkResult<V, R, T>),
...
impl<'a, E: Evaluator> ModeSwitchedBot<'a, E> {
    ...
    pub fn task_complete(&mut self, result: TaskResult<E::Value, E::Reward, E::Trace>) {
        match &mut self.mode {
...
impl Task {
    pub fn execute<E: Evaluator>(self, eval: &E) -> TaskResult<E::Value, E::Reward, E::Trace> {
        match self {
