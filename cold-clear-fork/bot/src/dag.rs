...
pub struct DagState<E: 'static, R: 'static, T: 'static> {
    board: Board,
    generations: VecDeque<Generation<E, R, T>>,
    root: u32,
    gens_passed: u32,
    use_hold: bool,
}
...
#[self_referencing]
struct Generation<E: 'static, R: 'static, T: 'static> {
    arena: Box<bumpalo::Bump>,
    #[borrows(arena)]
    #[not_covariant]
    data: GenerationData<'this, E, R, T>,
}

struct GenerationData<'c, E, R, T> {
    nodes: Vec<Node<'c, E, T>>,
...
impl<E: Evaluation<R> + 'static, R: Clone + 'static, T: Clone + Default + 'static> DagState<E, R, T> {
    ...
    fn find_and_mark_leaf_with_chooser(
        &mut self,
        mut chooser: impl for<'a> FnMut(&[Node<E, T>], &'a [Child<R>]) -> Option<&'a Child<R>>,
    ) -> Option<(NodeId, Board)> {
    ...
    pub fn update_known(&mut self, node: NodeId, children: Vec<ChildData<E, R, T>>) {
    ...
    pub fn update_speculated(
        &mut self,
        node: NodeId,
        mut children: EnumMap<Piece, Option<Vec<ChildData<E, R, T>>>>,
    ) {
        // make sure we weren't given a NodeId for an expired node. it could happen.
        if node.generation < self.gens_passed {
            return;
    ...
    fn get_gen_and_next(&mut self, gen: usize) -> [&mut Generation<E, R, T>; 2] {
    ...
    pub fn get_next_candidates(&self) -> Vec<MoveCandidate<E, T>> {
                ...
                if let Children::Known(_, children) = &gen.children {
                    if let Some(children) = &children[self.root as usize] {
                        for (i, child) in children.iter().enumerate() {
                            ...
                            let trace = child_gen.nodes[child.node as usize].trace.clone();
...
fn child_eval_fn<'a, E, R, T>(child_gen_nodes: &'a [Node<E, T>]) -> impl Fn(&Child<R>) -> Option<E> + 'a
where
    E: Evaluation<R>,
    R: Clone,
{
...
fn build_children<'arena, E: Evaluation<R> + 'static, R: Clone + 'static, T: Clone + 'static>(
    parent_arena: &'arena bumpalo::Bump,
    children_gen: &mut BorrowedMutFields<E, R, T>,
    mut children: Vec<ChildData<E, R, T>>,
    parent: u32,
    hold_allowed: bool,
) -> &'arena mut [Child<R>] {
    // sort best to worst
...
impl<E: 'static, R: 'static, T: 'static> Generation<E, R, T> {
