...
impl Evaluator for Standard {
    ...
    fn get_value(&self, val: &Value) -> i32 {
        val.value
    }

    fn into_standard_trace(&self, trace: &EvalTrace) -> Option<EvalTrace> {
        Some(trace.clone())
    }

    fn pick_move(
        &self,
        candidates: Vec<MoveCandidate<Value, EvalTrace>>,
        incoming: u32,
    ) -> MoveCandidate<Value, EvalTrace> {
        let mut backup = None;
    ...
    fn evaluate(
        ...
        move_time: u32,
        placed: Piece,
    ) -> (Value, Reward, EvalTrace) {
        self.evaluate_trace(lock, board, move_time, placed)
    }
}
...
impl Standard {
    pub fn evaluate_trace(
        ...
        let score_quarter = self.top_quarter * (highest_point - 15).max(0);
        let score_half = self.top_half * (highest_point - 10).max(0);
        let score_height = self.height * highest_point; // We'll add this later
        
        transient_eval += score_quarter;
        transient_eval += score_half;
        trace.height_penalty += score_quarter + score_half;

        let jeopardy_score = self.jeopardy
            * (highest_point - 10).max(0)
            * if self.timed_jeopardy { move_time } else { 10 }
            / 10;
        acc_eval += jeopardy_score;
        trace.jeopardy_penalty += jeopardy_score;
        ...
        for _ in 0..ts {
            ...
            trace.tslot_score[result.lines] += self.tslot[result.lines];
        ...
        let highest_point = *board.column_heights().iter().max().unwrap() as i32;
        let score_height2 = self.height * highest_point;
        transient_eval += score_height2;
        trace.height_penalty += score_height2;
        ...
        let well_s1 = self.well_depth * depth;
        transient_eval += well_s1;
        trace.well_score += well_s1;
        if depth != 0 {
            let well_s2 = self.well_column[well];
            transient_eval += well_s2;
            trace.well_score += well_s2;
        }

        if self.row_transitions != 0 {
            let rt_score = self.row_transitions
                * (0..40)
                    .map(|y| *board.get_row(y))
                    .map(|r| (r | 0b1_00000_00000) ^ (1 | r << 1))
                    .map(|d| d.count_ones() as i32)
                    .sum::<i32>();
            transient_eval += rt_score;
            trace.row_transitions += rt_score;
        }

        if self.bumpiness | self.bumpiness_sq != 0 {
            let (bump, bump_sq) = bumpiness(&board, well);
            let b_score1 = bump * self.bumpiness;
            let b_score2 = bump_sq * self.bumpiness_sq;
            transient_eval += b_score1;
            transient_eval += b_score2;
            trace.bumpiness_penalty += b_score1 + b_score2;
        }
            ...
            != 0
        {
            ...
            let c_score1 = self.cavity_cells * cavity_cells;
            let c_score2 = self.cavity_cells_sq * cavity_cells * cavity_cells;
            let o_score1 = self.overhang_cells * overhang_cells;
            let o_score2 = self.overhang_cells_sq * overhang_cells * overhang_cells;
            transient_eval += c_score1 + c_score2 + o_score1 + o_score2;
            trace.hole_penalty += c_score1 + c_score2 + o_score1 + o_score2;
        }

        if self.covered_cells | self.covered_cells_sq != 0 {
            let (covered_cells, covered_cells_sq) = covered_cells(&board);
            let cov_score1 = self.covered_cells * covered_cells;
            let cov_score2 = self.covered_cells_sq * covered_cells_sq;
            transient_eval += cov_score1 + cov_score2;
            trace.covered_penalty += cov_score1 + cov_score2;
        }
